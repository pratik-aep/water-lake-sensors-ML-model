"""One synthetic world for the whole system: lake sensor export (with faults and pollution events), air stations,
weather and lab results."""
...
from ..air_quality.simulate import simulate as simulate_air
...
def simulate(days: int = 455, seed: int = 31, stations: dict = STATIONS, chunk_days: int = 120):
    """Return (10-minute lake readings with injected faults and events, hourly weather, lab results, injected
    episodes, 15-minute air readings). Air and lakes share one city's weather, with wind, mixing height and humidity."""
    readings, weather, lab = simulate_lakes_and_lab(days, seed, stations, algae=True, coliform=True)
    air, weather = simulate_air(seed=seed + 5, weather=weather)
    ...
    return dirty, weather, lab, pd.concat(episodes, ignore_index=True), air
...
def main(argv=None):
    ...
    readings, weather, lab, episodes, air = simulate(args.days, args.seed)
    ...
    air.to_csv(args.out_dir / "air_readings.csv", index=False)
    print(f"Wrote {len(readings)} lake readings ({len(episodes)} injected faults and events), {len(air)} air readings, "
          f"{len(weather)} hours of weather and {len(lab)} lab results to {args.out_dir}")
